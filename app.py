...
if st.session_state.edit_index is not None:
    ...
    with col_chat:
        prompt_texto = st.chat_input("Pergunte qualquer coisa...")
        if prompt_texto:
    ...
    with col_voice:
        ...
        if audio_raw and "bytes" in audio_raw:
            aviso_transcricao = st.info("🎧 Processando sua voz...")
            texto_transcrito = transcrever_audio_nvidia(audio_raw["bytes"])
            aviso_transcricao.empty()
